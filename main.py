...
TELEGRAM_CHAT_ID   = os.environ.get("TELEGRAM_CHAT_ID", "").strip()   # numeric string

def tg_send(text: str, reply_markup: dict = None) -> bool:
    try:
        if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
            # Telegram disabled when env vars are not set
            return False
...
if __name__ == "__main__":
    ...
    # Start Telegram loop only when env vars are present
    t1 = None
    if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
        t1 = threading.Thread(target=telegram_loop, daemon=True)
        t1.start()
    else:
        print("[INFO] Telegram disabled. Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID to enable.")
